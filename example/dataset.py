...
def add_dataset_args(ap) -> None:
    ...
    ap.add_argument(
        ...
        help=("公共附加参数（自动追加到每条命令后）。"
              "用 | 分隔键值，例如：--rc-mode|1|--preset|1|--keyint|225|--bframes|15|--threads|4|--parallel-frames|5|"
...
def build_cmds_from_dataset(args, cfg) -> List[List[str]]:
    """
    输出：list[list[str]]，每个元素是一条编码器命令（已拆成 argv 形式）。
    - 仅构造 PASS=2
    - --stat-in = {stat-dir}/{seq}_qav1enc_pass1_{br}.log
    - --stat-out / --csv / --o 写到 out-dir，命名 {seq}_{enc}_pass2_{br}.*
    """
